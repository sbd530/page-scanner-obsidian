/**
 * Runs the vendored `page-scanner` command with a Node on this computer (./node.ts) and reads
 * its `--json` answer. A child process rather than an import, for the reason raycast/src/lib/cli.ts
 * gives: the CLI starts its daemon by running its own entry point again, which inside Obsidian
 * would be Obsidian. Run as a file of its own under Node, the bundle is `argv[1]` and starts
